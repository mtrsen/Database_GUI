...
class Project:
    ...
    def LoadTableReport(self):
        db = self.Connect()
        c = db.cursor()
        c.execute("SELECT DLocationName AS POILocation, Pcity AS City, Pstate AS State, MoldMIN, ROUND(MoldAVG, 0), MoldMAX, ROUND(AQMIN, 0), AQAVG, AQMAX, ( NumberDataPoints + NumberDataPoints1) AS NumberDataPoints, Flag AS Flagged FROM (SELECT DLocationName, Pcity, Pstate, Flag, MIN( DataValue ) AS MoldMIN, AVG(DataValue ) AS MoldAVG, MAX( DataValue ) AS MoldMAX, COUNT( * ) AS NumberDataPoints1 FROM DATAPOINT, POI WHERE DATAPOINT.DLocationName = POI.LocationName AND DType =  'Mold' AND Accepted = 'Yes' GROUP BY DLocationName) AS MOLD NATURAL JOIN (SELECT DLocationName, Pcity, Pstate, Flag, MIN( DataValue ) AS AQMin, AVG( DataValue ) AS AQAVG, MAX( DataValue )AS AQMAX, COUNT( * ) AS NumberDataPoints FROM DATAPOINT, POI WHERE DATAPOINT.DLocationName = POI.LocationName AND DType =  'Air Quality' AND Accepted = 'Yes' GROUP BY DLocationName) AS AQ")
        newtup = c.fetchall()
        print(newtup)
        newlist = list(newtup)
        for values in newlist:
